...
SCRIPT_PATH="/path/to/your/original/script"
...
if [ $((CURRENT_TIME - LAST_RUN)) -ge $INTERVAL ]; then
    ...
    osascript -e "tell application \"Terminal\" to do script \"$SCRIPT_PATH --rpc $RPC --chain-id $CHAIN_ID --amount-to-approve $AMOUNT_TO_APPROVE --amount-to-swap $AMOUNT_TO_SWAP --fetch-decimals $FETCH_DECIMALS --token-in $TOKEN_IN --token-out $TOKEN_OUT --verbose $VERBOSE --webhook $WEBHOOK\""
